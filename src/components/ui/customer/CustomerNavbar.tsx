...
export function CustomerNavbar({
  ...
  // Check if user is customer (role_id = 2)
  const isCustomer = user?.role_id === 2;
  // Check if user is admin (role_id = 1) or vendor (role_id = 4) or delivery agent (role_id = 5)
  const isStaffOrAdminOrVendor = user?.role_id === 1 || user?.role_id === 4 || user?.role_id === 5;

  // Only show chat for customers and authenticated users
  const shouldShowChat = token && isCustomer;
          ...
          >
            {darkMode ? (
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 12.79A9 9 0 1 1 11.21 3a7 7 0 0 0 9.79 9.79z" />
              </svg>
            ) : (
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v1m0 16v1m8.485-8.485h1M3.515 12h1m12.02 4.95l.707.707M6.343 6.343l.707.707M16.95 7.05l.707-.707M7.05 16.95l.707-.707M12 7a5 5 0 1 0 0 10a5 5 0 0 0 0-10z" />
              </svg>
            )}
          </button>
