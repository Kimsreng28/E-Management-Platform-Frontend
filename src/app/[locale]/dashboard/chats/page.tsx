...
export default function ChatPage({
  ...
  const getCustomersWithConversations = () => {
    const customerIdsInConversations = new Set(
    ...
    return customers.filter(customer =>
      customerIdsInConversations.has(customer.id)
    );
  };

  // Filter conversations and customers based on search
  const filteredConversations = conversations.filter(conv => {
    const otherParticipant = getOtherParticipant(conv);
    return otherParticipant?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      conv.latest_message?.body?.toLowerCase().includes(searchTerm.toLowerCase());
  });

  const customersWithConversations = getCustomersWithConversations();
  const filteredCustomers = customersWithConversations.filter(customer =>
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      {/* Sidebar */}
      <div className={`w-full md:w-80 lg:w-96 flex-shrink-0 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col fixed md:relative inset-y-0 left-0 transform ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} transition-transform duration-300 ease-in-out shadow-lg z-40`}>
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800">
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">{t.chatPage.customerSupport}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t.chatPage.chatWithCustomers}</p>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-3">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2 px-2">
              {t.chatPage.activeCustomers} ({filteredCustomers.length})
            </h3>
            <div className="space-y-2">
              {filteredCustomers.map(customer => (
                <div
                  key={customer.id}
                  className="flex items-center p-3 rounded-lg cursor-pointer transition-all hover:bg-gray-100 dark:hover:bg-gray-700"
                  onClick={() => findOrCreateConversation(customer.id)}
                >
                  <div className="relative">
                  ...
                  <div className="ml-3 overflow-hidden">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{t.chatPage.customer}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-3 border-t border-gray-200 dark:border-gray-700">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2 px-2">
              {t.chatPage.activeConversations} ({filteredConversations.length})
            </h3>
            <div className="space-y-2">
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex items-center bg-white dark:bg-gray-800">
          ...
          <div className="ml-3">
            <p className="font-medium text-gray-800 dark:text-white">{user?.name || t.chatPage.user}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {user?.role_id === 4 ? t.chatPage.vendor :
                user?.role_id === 5 ? t.chatPage.deliveryAgent :
                  t.chatPage.staff}
            </p>
          </div>
        </div>
        ...
        {activeConversation ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
            <div className="text-center max-w-md">
              ...
              <h3 className="text-xl font-semibold mb-2 text-gray-800 dark:text-white">{t.chatPage.welcomeToCustomerSupport}</h3>
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                {t.chatPage.selectCustomerDescription}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                  <h4 className="font-semibold text-gray-800 dark:text-white mb-2">{t.chatPage.forVendors}</h4>
                  <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                    <li>• {t.chatPage.answerCustomerInquiries}</li>
                    <li>• {t.chatPage.provideProductSupport}</li>
                    <li>• {t.chatPage.handleOrderIssues}</li>
                  </ul>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                  <h4 className="font-semibold text-gray-800 dark:text-white mb-2">{t.chatPage.forDeliveryAgents}</h4>
                  <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                    <li>• {t.chatPage.coordinateDeliveries}</li>
                    <li>• {t.chatPage.updateOrderStatus}</li>
                    <li>• {t.chatPage.resolveDeliveryIssues}</li>
                  </ul>
                </div>
              </div>
              <button
                className="mt-6 md:hidden px-4 py-2 bg-blue-500 text-white rounded-lg shadow-md"
                onClick={() => setIsMobileSidebarOpen(true)}
              >
                {t.chatPage.openCustomerList}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-gray-700">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-center space-x-4">
                ...
                {/* Speaker */}
                <button
                  className="w-14 h-14 rounded-full bg-gray-700 hover:bg-gray-600 flex items-center justify-center transition-all duration-200 shadow-lg shadow-gray-700/25"
                ...
                <button
                  onClick={handleEndCall}
                  className="w-16 h-16 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center transition-all duration-200 transform hover:scale-105 shadow-xl shadow-red-500/30"
                  title={t.chatPage.endCall}
                >
