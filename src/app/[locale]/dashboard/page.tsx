...
export default function DashboardPage({
    ...
    const fetchDashboardStats = async () => {
        try {
            ...
            const [statsRes, chartDataRes, recentOrdersRes] = await Promise.all([
                fetch(`${API_BASE_URL}/api/dashboard/stats`, {
                    headers: { Authorization: `Bearer ${token}` },
                }),
                fetch(`${API_BASE_URL}/api/dashboard/chart-data`, {
                    headers: { Authorization: `Bearer ${token}` },
                }),
            ...
            if (!chartDataRes.ok) {
                throw new Error(`Chart data failed: ${chartDataRes.status}`);
            }
            if (!recentOrdersRes.ok) {
                throw new Error(`Recent orders failed: ${recentOrdersRes.status}`);
            }
            ...
            // Set chart data with fallbacks and validation
            setRevenueData(chartData.revenueData || []);
            setCategoryData(chartData.categoryData || []);
            setSalesTrendData(chartData.salesTrendData || []);
            setOrderStatusData(chartData.orderStatusData || []);

            // Update stats state
            setStats({
                currentMonthRevenue: statsData.revenue?.current_month || 0,
                lastMonthRevenue: statsData.revenue?.last_month || 0,
                revenueChange: statsData.revenue?.change || 0,
                currentMonthOrders: statsData.orders?.current_month || 0,
                lastMonthOrders: statsData.orders?.last_month || 0,
                orderChange: statsData.orders?.change || 0,
                totalProducts: statsData.products?.total || 0,
                lastMonthProducts: statsData.products?.last_month || 0,
                productsChange: statsData.products?.change || 0,
                currentHourActive: statsData.active_customers?.current_hour || 0,
                previousHourActive: statsData.active_customers?.previous_hour || 0,
                activeCustomerChange: statsData.active_customers?.change || 0,
                totalCustomers: statsData.customers?.total || 0,
                recentOrders: recentOrdersData.orders?.map((order: any) => ({
                    id: order.id,
            ...
            <div className="space-y-1 px-1 sm:px-6 lg:px-1 lg:py-1 sm:space-y-6 md:px-6 sm:py-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
                        <h3 className="text-lg font-semibold mb-4">{t.dashboardPage.monthlyRevenueOrders}</h3>
                        <div className="h-80">
                                    ...
                                    <BarChart data={revenueData}>
                                        ...
                                        <Tooltip
                                            formatter={(value, name) => [
                                                ...
                                                name === 'revenue' ? t.dashboardPage.revenue : t.dashboardPage.orders
                                            ]}
                                        />
                                        <Legend />
                                        <Bar yAxisId="left" dataKey="revenue" fill="#8884d8" name={t.dashboardPage.revenue} />
                                        <Bar yAxisId="right" dataKey="orders" fill="#82ca9d" name={t.dashboardPage.orders} />
                                    </BarChart>
                                </ResponsiveContainer>
                            ) : (
                    ...
                    {/* Sales Trend Chart */}
                    <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
                        <h3 className="text-lg font-semibold mb-4">{t.dashboardPage.weeklySalesTrend}</h3>
                        <div className="h-80">
                            {salesTrendData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={salesTrendData}>
                                        ...
                                        <YAxis />
                                        <Tooltip
                                            formatter={(value) => [`$${Number(value).toLocaleString()}`, t.dashboardPage.sales]}
                                        />
                                        <Legend />
                                        <Line
                                            type="monotone"
                    ...
                    <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
                        <h3 className="text-lg font-semibold mb-4">{t.dashboardPage.categoryDistribution}</h3>
                        <div className="h-80">
                            {categoryData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        ...
                                        <Tooltip
                                            formatter={(value) => [`${Number(value).toFixed(1)}%`, t.dashboardPage.percentage]}
                                        />
                                        <Legend />
                                    </PieChart>
                                </ResponsiveContainer>
                                ...
                                <div className="flex items-center justify-center h-full text-gray-500">
                                    {t.dashboardPage.noCategoryData}
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {/* Orders Status Chart */}
                    <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
                        <h3 className="text-lg font-semibold mb-4">{t.dashboardPage.orderStatusDistribution}</h3>
                        <div className="h-80">
                            {orderStatusData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={orderStatusData}>
                                        ...
                                        <YAxis />
                                        <Tooltip />
                                        <Bar dataKey="count" fill="#ffc658" name={t.dashboardPage.count} />
                                    </BarChart>
                                </ResponsiveContainer>
                            ) : (
                                <div className="flex items-center justify-center h-full text-gray-500">
                                    {t.dashboardPage.noOrderStatusData}
                                </div>
                            )}
                        </div>
                    </div>
