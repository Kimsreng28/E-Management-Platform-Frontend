      ...
      <aside className="w-64 bg-[#F9F9F9] text-black flex flex-col">
        <div className="flex items-center justify-between p-6 font-['Inria_Sans']">
          ...
          <div className="relative cursor-pointer bg-white w-8 h-8 flex items-center justify-center rounded-lg shadow-sm hover:shadow-md transition">
            ...
            {/* Notification badge */}
            {/* <span className="absolute -top-1 -right-1 bg-[#f53003] text-white text-xs font-bold w-4 h-4 rounded-full flex items-center justify-center shadow"></span> */}
          </div>
        </div>
        <nav className="flex-1  space-y-2 px-4">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex items-center gap-3 bg-gray-100 border border-gray-200 rounded-lg px-4 py-2 hover:bg-gray-900 hover:text-white transition",
